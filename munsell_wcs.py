...
def build_wcs_map():
	...
	for row in wcs_csv:
		...
		chip = { 'number' : row[0],
			 ...
			 'chroma' : int(row[3]),
			 'value' : float(row[5]),
			 'Lab' : [float(row[6]), float(row[7]), float(row[8])] }
...
def main():
	...
	pic = plt.imshow(np.flipud(grid), interpolation='nearest')
	#plt.gca().invert_yaxis()
	plt.yticks([0, 1, 2, 3, 4, 5, 6, 7], ['B', 'C', 'D', 'E', 'F', 'G', 'H', 'I'])
	plt.title('Chroma of WCS')
	cbar = plt.colorbar(pic, shrink=0.27)
	cbar.set_ticks(xrange(2, 18, 2))
	plt.savefig('Chroma WCS values.pct', dpi=600)
	plt.show()
